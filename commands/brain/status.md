...
**S0 — Preconditions**: `.brain/` exists; `.brain/.graphbrain-version` present. (If not: same npx-init message.)

**S1 — Gather**:

- Page counts per kind: walk `.brain/code/`, `.brain/concepts/`, `.brain/decisions/`; count `.md` files.
- Hooks installed: read `.claude/settings.local.json`; count entries with `id` starting `graphbrain:`.
- Last 5 log entries: `tail -5 .brain/log.md | grep "^## \["` (grep-parseable per PRD #15).
- Learn state: read `.brain/.graphbrain-learn-state` (default `missing` → display as `off (default)`).
...
- Intent-routing state (M#10c): read `.brain/.graphbrain-intent-routing-state` (default `missing` → display as `off (default)`). Malformed → `off (malformed)`.

**S2 — Format the dashboard**:

```
graphbrain status (v<version>)
...
Hooks installed:     <count> [graphbrain:pre:verified-guard, graphbrain:pre:observe, graphbrain:post:stale-detect]
