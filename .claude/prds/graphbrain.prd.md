...
graphbrain is the LLM-Wiki pattern (per the reference document the user provided) **adapted from documents/research to codebases**. Three explicit layers, with clear ownership and immutability rules:

| Layer | What | Owned by | Mutability contract |
|---|---|---|---|
| **1. Raw sources** | The codebase itself — every file under the repo root, excluding `.brain/` | The developer + their agent's normal coding work | The brain layer **never** modifies source code. Source is read-only from graphbrain's perspective. Source edits happen through the operator's normal workflow (the agent's Edit/Write tools); the PostToolUse hook then *reacts* to those edits by marking pages STALE. |
| **2. The wiki** (`.brain/`) | Folder-mirrored code pages, cross-cutting concept pages, decisions/ADRs, overview, index, log, status | graphbrain skills (init/ingest/query/lint) — the LLM writes, never the human | The operator reads; the agent writes. Humans can manually edit pages (they're just markdown), but the contract is that the agent maintains them. |
| **3. The schema** (the `## graphbrain` block in `CLAUDE.md`) | Layout description, conventions, ingest/query/lint operating rules, page-type taxonomy | Co-evolved by operator + LLM over the project's lifetime | `/brain init` is **re-runnable** — it updates the *managed region* (delimited by `<!-- graphbrain:begin -->` / `<!-- graphbrain:end -->`) without destroying any human edits inside or outside that region. The operator can edit conventions inside the managed region and re-running init preserves their changes (template-merge, not template-replace). |
...
| Time-to-first-value | < 5 min from `npx graphbrain init` to first useful wiki page | Wall-clock during onboarding test on a fresh machine |
...
| 8 | Slash commands: **`/brain *`** primary, `/graphbrain *` registered as an alias | Short, matches `.brain/` directory; alias keeps unambiguous form for tab-complete |
...
| 20 | **Prompt Defense Baseline: vendored from ECC into the `## graphbrain` CLAUDE.md schema block; agents reference it (don't re-copy) via "Read `.brain/protocol.md` for the prompt defense baseline" instruction** | ECC's prompt-defense block is duplicated across 60+ agent files (drift risk). graphbrain's runtime-read pattern is cleaner. Graphbrain stores once, references everywhere. |
| 21 | **Skills tier model: `skills/{behavioral,ingestion,core,detected,available}/`** (graphbrain pattern) — *not* a flat `skills/*/` list | ECC's 232 flat skills make install scope, deprecation, and "what gets loaded by default" impossible to reason about. graphbrain's tiers give every skill a clear lifecycle: `behavioral` always loaded, `ingestion` loaded during `/brain ingest`, `core` always available, `detected` auto-installed by stack detection, `available` opt-in. Graphbrain ships only a handful of skills — but seeds the structure from day one so it scales without restructuring. |
...
| 26 | **Error recovery (every graphbrain agent): Tier 1 retry once → Tier 2 escalate to operator with structured "blocked" report** | graphbrain's full 4-tier model (retry → reflect → kill+report → escalate) is expensive at graphbrain's scale. The "reflect" tier (ask the agent why it failed) can chew through tokens. Graphbrain: retry once, then surface a "blocked: agent couldn't complete X because Y; operator action: Z" message. Documented in `agents/README.md` so every shipped agent follows the same pattern. |
| 27 | **E2E shell test from Milestone #1**: `tests/e2e-test.sh` runs `npx graphbrain init` in a tmpdir and asserts `.brain/` scaffold, `.claude/commands/brain.md` written, settings.local.json hooks merged correctly | graphbrain's `tests/e2e-test.sh` is the right shape — fast, no LLM calls, deterministic. Graphbrain ships this from M#1 so onboarding validation is automatable and every milestone-completion gate runs it. |
| 28 | **Distribution: npm package** (`npx graphbrain init` or `npm install -g graphbrain`) — **not** a Claude Code plugin | npm is the "normal" install path developers expect; graphbrain proves the model works for harness-config tooling. Plugin packaging may revisit post-MVP if there's discovery demand, but it's not a precondition for shipping. |
...
| 32 | **Hooks ownership via `id:` prefix** — graphbrain owns only entries in `settings.local.json` whose `id` starts with `graphbrain:` (e.g., `graphbrain:pre:edit-write:stale-detect`). Merge preserves all non-graphbrain entries unchanged; re-init replaces graphbrain-prefixed entries in place (no duplicates); future `graphbrain uninstall` removes only graphbrain-prefixed entries. | Settings.local.json's hooks structure is `{ hooks: { PreToolUse: [...] } }` — there's no namespace key we can write under. Without a stable ownership marker, init can't tell its own entries from user/ECC/other-tool entries, can't safely upgrade, and can't safely uninstall. `id:` prefix is the canonical pattern (ECC uses `ecc:`, others use their own). |
| 33 | **`.brain/.graphbrain-version` marker file shipped from M#1** — one line containing the graphbrain version that scaffolded the repo (e.g., `0.1.0`). Updated only by `graphbrain init` itself. | Without a version marker, a future graphbrain release can't tell v0.1 state apart from v0.3 state without heuristics. Marker file is the cheap enabler that makes any future migration possible. Costs almost nothing; defers the migration script itself to post-MVP. |

## Scope

**MVP** — An **npm package** that, on `npx graphbrain init` (project-local by default; `--global` for `~/.claude/`), scaffolds `.brain/` in the cwd, writes `/brain` slash commands into `.claude/commands/`, and merges graphbrain hooks into `.claude/settings.local.json`. After init: no bash scripts the user runs, no setup wizard, no external extractor — every operation is `/brain *` slash commands directed by LLM agents inside Claude Code.
...
└── CLAUDE.md                  # gets a `## graphbrain` schema block (managed region) appended
```
...
1. **`init`** — Scaffolds `.brain/`, writes the `## graphbrain` schema block to `CLAUDE.md` inside a managed region (`<!-- graphbrain:begin -->` / `<!-- graphbrain:end -->`). **Re-runnable**: subsequent runs update only the managed region's *template-driven* sections; operator edits inside the managed region (e.g., project-specific conventions) and outside it are preserved (template-merge, not template-replace). Opt-out: `--private` flag adds `.brain/` to `.gitignore`.
...
3. **`PreToolUse` observer (continuous-learning)** — Narrow scope: only captures Edit/Read on tracked source files + UserPrompt tokens matching codebase symbols. Async, ≤10s timeout, exits 0 on error (never blocks tool execution). Writes to `~/.local/share/graphbrain/projects/<hash>/observations.jsonl`.

### Continuous-learning model (reuses ECC v2.1)
- **Atomic instincts** — small behaviors with confidence scores (0.3–0.9).
- **Project-scoped** by default at `~/.local/share/graphbrain/projects/<git-remote-hash>/instincts/personal/`.
...
- **Promotion pipeline** — instincts seen in N≥2 projects with avg confidence ≥0.8 become candidates for `~/.local/share/graphbrain/instincts/global/`.
...
- The other ~230 ECC skills — graphbrain borrows only the harness primitives (skill format, command frontmatter shape, hook entry shape, continuous-learning-v2 core) from ECC's reference implementation.
...
|---|---|---|---|---|
| 1 | npm package skeleton + init | `npx graphbrain init` scaffolds `.brain/` in cwd, writes `/brain` + `/graphbrain` slash commands into `.claude/commands/`, merges graphbrain hooks block into `.claude/settings.local.json` — project-local by default; `--global` for `~/.claude/`; `--force` overrides; idempotent on repeat | complete | [.claude/plans/graphbrain.plan.md](.claude/plans/graphbrain.plan.md) |
| 2 | Init + schema scaffolding | `/brain init` creates full `.brain/` layout (code/, concepts/, decisions/, overview.md, index.md, log.md, decisions.md, status.md); appends `## graphbrain` schema block to CLAUDE.md; idempotent | complete | [.claude/plans/graphbrain-m2.plan.md](.claude/plans/graphbrain-m2.plan.md) |
| 3a | Ingest pipeline — single-file end-to-end | `/brain ingest <single-file-path>` invokes the ingester agent which writes `.brain/code/<path>.md` using the page-format template; updates index/status/log; idempotent on unchanged source | complete | [.claude/plans/graphbrain-m3a.plan.md](.claude/plans/graphbrain-m3a.plan.md) |
| 3b | Ingest pipeline — folder + concept pages + linker | `/brain ingest <folder>` walks the folder, ingests each file via the M#3a ingester; linker agent creates concept pages for cross-cutting ideas and wires bidirectional wikilinks | complete | [.claude/plans/graphbrain-m3b.plan.md](.claude/plans/graphbrain-m3b.plan.md) |
| 3c | Ingest pipeline — tiered auto-prioritize | `/brain ingest` (no args) invokes the planner agent: reads cached stack detection, groups files into 3 tiers using generic globs, presents the plan with per-tier cost estimates, gates each tier (yes/no/show-files/cancel/--yes), delegates each confirmed tier to the M#3b folder procedure; linker runs after each tier for incremental visibility | complete | [.claude/plans/graphbrain-m3c.plan.md](.claude/plans/graphbrain-m3c.plan.md) |
| 3d | Ingest pipeline — stack-aware page templates (detected/* skills) | 4 detected/* skills (react, python, go, typescript) ship with per-stack code-page extras using an INHERITANCE pattern (extras append AFTER the generic 5 sections — never replace). Ingester picks matching skill when project's detected stack + source file's extension both match; multiple skills can apply per file (e.g., `.tsx` in React+TS gets TypeScript extras + React extras in registry order). registry.json populated. Stack-aware tier-glob overrides for M#3c deferred to future polish per sweep finding E7. | complete | [.claude/plans/graphbrain-m3d.plan.md](.claude/plans/graphbrain-m3d.plan.md) |
| 4 | Staleness model — tiers 1+2 (PostToolUse hook + PreToolUse verified-guard) | PostToolUse `graphbrain hook stale-detect` flips `.brain/code/<edited>.md` to STALE; walks all `.brain/**/*.md` to find wikilinks AND `sources:` array references; flips matching pages STALE too. PreToolUse `graphbrain hook verified-guard` blocks writes to `status: VERIFIED` pages without `--force` (structural layer of dual-layer guardrail, PRD #19). Tier 3 (lint contradiction-check) lands in M#6; tier 4 (query-time refresh) lands in M#5. | complete | [.claude/plans/graphbrain-m4.plan.md](.claude/plans/graphbrain-m4.plan.md) |
| 5 | Query helper — `/brain query` with tier-4 staleness | `/brain query "<question>" [--thorough] [--no-refresh]` invokes Researcher-pattern query agent; reads `.brain/index.md` pointer-first; selects 1–3 candidates (≤5 with --thorough; hard cap 5); per-candidate hash compare promotes STALE→FRESH on match (resolves M#4's conservative flips), delegates to M#3a ingester for refresh on mismatch; cites BOTH brain pages (`[[code/...]]`) AND source paths (`src/file.ts:42` only when line actually read); structured Answer/Citations/Brain freshness output; grep-parseable log entry. Closes tier 4 of the 4-tier staleness model. | complete | [.claude/plans/graphbrain-m5.plan.md](.claude/plans/graphbrain-m5.plan.md) |
| 6 | Lint pass — `/brain lint` (closes tier 3 staleness) | `/brain lint [--fix] [--yes] [--include-contradictions]` invokes Verifier-pattern agent. 4 categories: (1) Defects — stale (hash-verified true vs false-positive), broken wikilinks, page-size violations, orphan source files, schema-drift in CLAUDE.md; (2) Gaps — missing concept pages (heuristic ≥2 mentions), stub/TBD pages, graph-orphan code pages; (3) Contradictions (LLM-driven, opt-in, cost-gated at >$0.50) — page-vs-source behavior drift = tier 3 of 4-tier staleness model; (4) Suggested questions — forward-looking operator actions. `--fix` delegates batch refresh to M#3a (code) + M#3b (concept folders) with confirmation. Always exits 0 in v0.1; severity-coded exits deferred to post-MVP. | complete | [.claude/plans/graphbrain-m6.plan.md](.claude/plans/graphbrain-m6.plan.md) |
| 7 | Continuous-learning observer | Sixth and final agent pattern (Observer; locks the roster). Three components ship: (1) PreToolUse hook `graphbrain:pre:observe` — fast, async, no-LLM, exits 0 always, silently no-ops unless `.brain/.graphbrain-learn-state == on`; (2) Observer agent + `core/learn` skill — read-only consolidator invoked via `/brain learn consolidate`, deterministic pattern counting (frequency ≥3 → instinct), writes to `<XDG>/graphbrain/projects/<git-hash>/{observations,instincts}.jsonl`; (3) `/brain learn {on\|off\|status\|consolidate}` + `/brain status` slash commands. Default OFF per PRD #4 (opt-in). Privacy by design: whitelist-enforced 4 fields only (`ts`, `tool`, `path?`, `status`); never captures tool outputs / prompts / file contents. LLM-driven distillation + export/import deferred to v0.2. **Graphbrain v0.1 is feature-complete after M#7.** | complete | [.claude/plans/graphbrain-m7.plan.md](.claude/plans/graphbrain-m7.plan.md) |
| 8 | Dogfood + measure — validation framework | Ships the harness: `scripts/dogfood/install-validate.sh` (real-world install test on graphbrain itself; 22 assertions) + `scripts/dogfood/static-baseline.sh` (gathers shipped-artifact metrics → `.claude/validation/v0.1-static-baseline.md`; recursion-guarded via CODEBRAIN_BASELINE_NESTED env) + `scripts/dogfood/MANUAL-MEASUREMENTS.md` (5-section operator procedure: token-reduction A/B, freshness drift, wikilink precision, time-to-first-value, continuous-learning lift). `.claude/validation/v0.1-baseline.md` is the report template the operator fills as evidence arrives. README's Dogfood + validate section links the entry points. **Full LLM-driven validation is operator-driven over time** — M#8 ships the framework, not the measurements. | complete | [.claude/plans/graphbrain-m8.plan.md](.claude/plans/graphbrain-m8.plan.md) |
...
| Overlap with ECC's continuous-learning-v2 creates two competing instinct stores if user installs both | Med | Med | Graphbrain instincts in their own XDG namespace under `graphbrain/`; document interop; long-term factor v2 core out so both consume it |
| `graphbrain init` corrupts existing `.claude/settings.local.json` when merging the hooks block | Med | High | Init script does JSON-aware merge (not text concat); preserves existing hooks; idempotent (re-running with same input is a no-op); always writes a `.bak` before edit; `--force` is the only path that overwrites |
| User runs `graphbrain init` outside a project directory and pollutes `~` | Low | Med | Init detects cwd; if no git root + no package.json + no pyproject.toml, refuses with explicit error; `--global` is the only path that writes to `~/.claude/` |
...
v0.1 shipped 11 sub-milestones with graphbrain feature-complete (commits 4f07b13 through 0a20551). v0.1.1 (commit 4566312) addressed **Gap A** from operator dogfood (framework-level detection + ECC bridge architecture). v0.1.2 (this revision) shipped `.brain/llms.txt` (AEO-convention agent-portable site map) + SKILL.md reciprocity test, informed by independent agentctx-idea research. Two milestones remain on the post-MVP roadmap, both addressing additional operator-discovered gaps:

| # | Milestone | Outcome | Status | Plan |
|---|---|---|---|---|
| 9 | Framework-detection + ECC-bridge full architecture (Gap B) | Beyond v0.1.1's preview: ship the runtime mechanism that ACTUALLY loads `expert_skills:` declared in `detected/*/SKILL.md` at ingest/query/lint time. Today the bridge is declarative (registry + slash-command body documents the contract); v0.2 makes it operative (probe the harness, load the ECC skill, apply its guidance throughout the session). Also: extend coverage to vue, rails, flask, koa, hapi + add `expert_skills:` bridges to the four M#3d skills (react, typescript, python, go). | pending | `.claude/plans/graphbrain-m9.plan.md` |
| 10 | Spec-first workflow + intent routing + discovery loop + agent-readability hardening (Gap C + external research) | New `/brain spec "<intent>"` verb that orchestrates ECC's `plan-prd` → `plan` → `santa-loop` convergence into a guided spec-then-plan-then-execute flow. Plus a behavioral-skill update: when operator's prompt expresses feature intent ("add", "build", "create", "implement"), the graphbrain meta-skill instructs the agent to suggest `/brain spec` BEFORE jumping to code. Plus a built-in discovery-loop pattern that codifies the iterative sweep behavior we organically discovered during the v0.1 build session. **M#10d (new)** adds agent-readability hardening from independent agentctx-idea research: `supersedes`/`superseded_by` frontmatter (pink-elephant fix), `.brain/CHANGELOG.md` (compound learning made visible), `skills/behavioral/wiki-reading-principles/SKILL.md` (3-tier always/ask/never). | pending | `.claude/plans/graphbrain-m10.plan.md` |

**Ordering recommendation**: ship **M#10 before M#9**. M#10's content discipline is corroborated by independent research (agentctx-idea — 17 sources / 75+ concepts; see M#10 plan "Evidence base"). M#9's bridge runtime refines an already-shipping declarative pattern. If implementation order is constrained by M#10's dependency on cross-plugin skill invocation, ship M#9 Tasks 1-2 first, then M#10, then the rest of M#9. M#10d (the agent-readability sub-milestone) is the smallest, independent of M#10a–c, and a natural "ship-first" candidate.

Both v0.2 milestones are documented in plan files; implementation is operator-scheduled.

Operator-discovered gaps from M#8 dogfood (the empirical source of M#9 + M#10a–c) are captured in `.claude/validation/v0.1-baseline.md` "Operator-discovered gaps" section. M#10d's evidence base is the agentctx-idea research wiki (external; not in repo).

---
*Status: DESIGN-LOCKED — 8 original open questions resolved; ready for `/plan .claude/prds/graphbrain.prd.md` on Milestone #1. v0.1 feature-complete (commit 0a20551); v0.1.1 framework-detection patch shipped (commit 4566312); v0.2 roadmap defined.*
