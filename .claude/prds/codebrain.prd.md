...
| 3d | Ingest pipeline — stack-aware page templates (detected/* skills) | 4 detected/* skills (react, python, go, typescript) ship with per-stack code-page templates; ingester picks the stack-specific template when matching detection signals are present; stack-aware tier-glob overrides land in commands/brain.md | pending | — |
