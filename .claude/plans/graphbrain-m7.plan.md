# Plan: graphbrain — Milestone #7 (Continuous-learning observer)
...
1. **PostToolUse hook** (`graphbrain:pre:observe`) fires on every Edit/Read/Write/Bash/etc. tool call, appends a minimal observation record (`{ts, tool, path?, status}`) to `<XDG>/projects/<git-hash>/observations.jsonl`. Fast (<100ms), async, no LLM, exits 0 always.
...
After M#7 ships, **graphbrain v0.1 is feature-complete**: 6 agent patterns, 5 skill tiers, 4-tier staleness, complete ingest/query/lint/learn loop, dogfood-validated.

## Patterns to Mirror

| Category | Source | Pattern |
|---|---|---|
| Hook script shape | `scripts/hooks/stale-detect.js` (M#4) | Shebang + stdin JSON read + exit-0-always + stderr-with-[graphbrain]-prefix-on-errors |
| Hook subcommand dispatch | `bin/graphbrain.js` HOOK_SUBCOMMANDS (M#4) | Add `observe` to the existing array; `npx graphbrain hook observe` is the install-time invocation |
| Hook entry in init.js | `scripts/init.js` codebrainOwnedHooks (M#4) | Add a third entry: `graphbrain:pre:observe` with `matcher: "*"`, `async: true`, `timeout: 10` |
...
- **J1 — Per-project toggle file**: `.brain/.graphbrain-learn-state` containing literal `on` or `off`. **Default `off`** per PRD Design Decision #4 (continuous-learning is opt-in). The observe hook reads this file each invocation; if `off` or missing → exits silently. Operator changes state via `/brain learn on` and `/brain learn off`.
...
- **J3 — Project-hash for XDG path**: SHA-256 of `git remote get-url origin` (first 12 chars). If no remote: SHA-256 of cwd path. Same scheme as ECC v2.1 for consistency. Path: `<XDG_DATA_HOME or ~/.local/share>/graphbrain/projects/<hash>/`.
...
- **J7 — Sixth and final agent pattern**: Observer. Completes the roster: Meta (graphbrain), Generator (ingester, detected/*), Reviewer (linker, page-format, concept-extraction), Planner (planner), Researcher (query), Verifier (verifier), Observer (observer). All 7 used.
...
| `bin/graphbrain.js` | UPDATE | Add `observe` to `HOOK_SUBCOMMANDS`; updates help text |
| `scripts/init.js` | UPDATE | Add third entry to `codebrainOwnedHooks`: PreToolUse `graphbrain:pre:observe` with `matcher: "*"`, `async: true`, `timeout: 10`. Hook entries are partitioned by id-prefix per M#1's merge logic — adding doesn't disturb the M#4 entries. |
...
| `.claude/prds/graphbrain.prd.md` | UPDATE | M#7 row → complete. **Graphbrain v0.1 is feature-complete after this commit.** |
...
- `xdgProjectDir(cwd)` — returns `<XDG_DATA_HOME>/graphbrain/projects/<hash>/`. Creates dir if missing.
- `learnToggleState(cwd)` — reads `<cwd>/.brain/.graphbrain-learn-state`. Returns `"on" | "off" | "missing"`.
...
2. Check `.brain/.graphbrain-learn-state` via lib/observations. If `off` or `missing` → exit 0 silently. If `on` → continue.
...
### Task 3: bin/graphbrain.js update — add `observe` subcommand
...
```
graphbrain hook observe — PreToolUse: collect minimal tool-use observations
...
- **NEVER run when toggle is `off`** — check `.brain/.graphbrain-learn-state` before reading.
...
- **ALWAYS respect the per-project toggle**: if `.brain/.graphbrain-learn-state` is `off`, abort with a notice telling the operator to flip `on` first.

Error recovery: Tier 1 retry / Tier 2 blocked-report. max_iterations 5.

### Task 6: skills/core/learn/SKILL.md (fourth core skill)

Frontmatter `tier: core`, `pattern: Observer`, `related_skills: [behavioral/graphbrain, core/lint]`.
...
- **Le1 — Preconditions**: `.brain/` exists; `.graphbrain-version` present
- **Le2 — Dispatch to subcommand procedure**:
  - `on`: print privacy notice → write `.brain/.graphbrain-learn-state` with content `on` → confirm
...
### Task 8: Update commands/graphbrain.md (alias parity)
...
- Run init in tmpdir; assert `settings.local.json` PreToolUse contains entry with `id: graphbrain:pre:observe`, `matcher: *`, `async: true`, `command` includes `npx graphbrain hook observe`
- Total graphbrain hooks now: 3 (verified-guard + stale-detect + observe)
...
- `bin/graphbrain.js hook observe` is a known subcommand
- Fixture test: in a dir without `.brain/.graphbrain-learn-state` → observe exits 0 silently; create the file with `off` → still silent; create with `on` + append a record → check observations.jsonl
...
`.claude/prds/graphbrain.prd.md` M#7 row → `complete`. Add footnote: "Graphbrain v0.1 is feature-complete after M#7. All 11 sub-milestones green; 6 agent patterns; 5 skill tiers; 4-tier staleness; complete loop."
...
# 3. observe.js wired
node bin/graphbrain.js hook 2>&1 | grep -q observe
...
# 5. init.js writes 3 graphbrain entries (2 existing + 1 new)
node -e "
...
const cb = all.filter(e => e && typeof e.id === 'string' && e.id.startsWith('graphbrain:'));
if (cb.length !== 3) { console.error('expected 3 graphbrain hooks, got', cb.length); process.exit(1); }
"
...
diff <(awk '/^## When `\$ARGUMENTS` starts with `learn`$/{flag=1} flag' commands/brain.md) \
     <(awk '/^## When `\$ARGUMENTS` starts with `learn`$/{flag=1} flag' commands/graphbrain.md)
diff <(awk '/^## When `\$ARGUMENTS` is just `status`$/{flag=1} flag' commands/brain.md) \
     <(awk '/^## When `\$ARGUMENTS` is just `status`$/{flag=1} flag' commands/graphbrain.md)
...
| XDG path doesn't exist on Windows | Low | `os.homedir()` works cross-platform; `<homedir>/AppData/Local/graphbrain/` is the Windows equivalent. Test in T35 if possible. |
| `npx graphbrain hook observe` overhead on every tool call | Low | async + timeout: 10 + exits-0-always; hook orchestration is microseconds; observation write is sub-ms. The PostToolUse hook from M#4 already proves this is fine. |
...
- [ ] Validation §2–§8 pass
- [ ] PRD M#7 row → complete (graphbrain v0.1 feature-complete)
