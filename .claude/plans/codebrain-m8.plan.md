# Plan: codebrain — Milestone #8 (Dogfood + measure)
...
Validate whether codebrain v0.1 actually works as the PRD claims. Two-part deliverable:

1. **Automatable now** (this session): static install validation on codebrain itself; verify the scaffold is correct, hooks land in the right shape, npm pack ships the right files, the slash-command surface dispatches correctly.
...
After M#8 ships, codebrain v0.1 is **publishable to npm** — the framework can collect evidence over time, but the codebase itself is feature-complete + validated as far as bash can validate.
...
- **H1 — Dogfood on codebrain itself first**: the most honest test. If codebrain can't usefully ingest its own scaffold, it's not ready for any other repo. M#8's automatable portion runs against the codebrain repo at `/Users/dev/Desktop/Project/OSS/idea/codebrain/`.
...
- **H5 — The 3 target repos are explicit**: PRD says "3 sample repos (codebrain itself + 2 OSS targets)". The 2 OSS targets are the natural choices given context: graphify and graphbrain at `/Users/dev/Desktop/Project/OSS/idea/{graphify,graphbrain}/`. ECC is also a reasonable target but is larger. Pick graphify + graphbrain as the 2 OSS targets; document both.
- **H6 — `validate` CLI verb deferred to post-MVP**: tempting to add `npx codebrain validate` that runs the static checks. Defer — it duplicates the bash script's purpose without adding much value for v0.1. The bash scripts ship as documented entry points.

## Files to Change

| File | Action | Why |
|---|---|---|
| `scripts/dogfood/install-validate.sh` | CREATE | Bash script: runs `node bin/codebrain.js init` in a tmp clone of codebrain itself, asserts the scaffold is correct, hooks land in `.claude/settings.local.json` with proper `codebrain:*` ids, expected files present. Effectively a "real-world install" test on top of the e2e test. |
...
| `tests/e2e-test.sh` | UPDATE | T30 — dogfood scripts exist + are executable + reference the right paths. T31 — `install-validate.sh` runs successfully against codebrain (smoke). T32 — `static-baseline.sh` runs successfully and produces a valid report file. |
| `.claude/prds/codebrain.prd.md` | UPDATE | M#8 row → `complete` (the framework ships); add a footnote that full validation is operator-driven over time. |
...
- `npx codebrain validate` CLI verb (H6 — defer)
...
- Create a tmpdir; copy codebrain into it (or clone — `git clone $CODEBRAIN_ROOT $TMPDIR` keeps it git-aware)
- Create a "fake user repo" tmpdir with `.git/` so the project-dir guard passes
- Run `node $CODEBRAIN_COPY/bin/codebrain.js init` from the fake user repo
...
- Specifically assert:
  - `.brain/` complete (code/, concepts/, decisions/, .codebrain-version, the 5 .md files)
  - `.claude/commands/brain.md` + `codebrain.md` present + match the source templates byte-for-byte
  - `.claude/settings.local.json` has both `codebrain:` hook entries with correct shape
...
## Shipped artifacts
  ...
  commands/codebrain.md:          <line count>
...
## Hooks
  PreToolUse codebrain entries: <count> [ids]
  PostToolUse codebrain entries: <count> [ids]
...
- Repos: codebrain (this one), graphify (`/Users/dev/Desktop/Project/OSS/idea/graphify`), graphbrain (`/Users/dev/Desktop/Project/OSS/idea/graphbrain`)
- For each repo:
  1. `npx codebrain init`
  2. `/brain init`
...
- Fresh machine (or remove `.brain/`, `.claude/commands/brain*`, settings.local.json codebrain entries)
- Stopwatch start
- `npx codebrain init`
- `/brain init`
...
Template content:
- Header (date, codebrain version, summary status)
...
`.claude/prds/codebrain.prd.md` M#8 row → `complete` with plan link. Add footnote: "Validation framework ships; full LLM-driven validation is operator-driven over time via scripts/dogfood/MANUAL-MEASUREMENTS.md."
...
| Operator measurements diverge wildly across repos (e.g., codebrain shows 60% token reduction, graphify shows 20%) | Possible | Report each repo separately in the baseline; the AVERAGE matters less than the per-repo number. Repos with poor results are signals for M#3d-style stack-aware improvements. |
