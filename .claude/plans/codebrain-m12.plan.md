# Plan: codebrain — Milestone #12 (Slash-command namespacing — `/brain:<verb>`)
...
Today every codebrain slash command flows through one of two monolithic files:

- `commands/brain.md` (1183 lines after v0.1.2) — single dispatcher; routes on the first token of `$ARGUMENTS`
- `commands/codebrain.md` (1183 lines) — byte-identical alias mirror (modulo title/help-text headers)
...
├── codebrain/        ← byte-identical mirror per-verb
│   └── (same)
├── brain.md          ← reduced to no-arg help / disambiguator
└── codebrain.md      ← reduced to no-arg help / disambiguator
```
...
| Alias byte-identity assertion | T36 (Step 4b.2 parity) + T37 (llms.txt refresh section parity) | Generalize: every `commands/brain/<verb>.md` and `commands/codebrain/<verb>.md` PROCEDURE body is byte-identical (headers/titles may differ) |
...
| File | Action | Why |
|---|---|---|
| `commands/brain/init.md` (NEW) | CREATE | Extract `## When $ARGUMENTS is init` (lines ~50–135 of brain.md) into its own file. Frontmatter `description: codebrain — init the brain in this project`. Procedure body byte-identical with current. |
...
| `commands/codebrain/<verb>.md` × 6 (NEW) | CREATE | Byte-identical mirror per verb (modulo header/title divergence as today). |
| `skills/ingestion/llms-txt/SKILL.md` (NEW) | CREATE | New skill extracting v0.1.2's "How to refresh `.brain/llms.txt`" reference section. Tier `ingestion`. Each verb file references it via `Read the skill at skills/ingestion/llms-txt/SKILL.md before refreshing`. Replaces the current inline duplication across brain.md / codebrain.md. |
...
| `commands/codebrain.md` | REDUCE | Same reduction. |
| `scripts/init.js` | UPDATE | `copyTemplate` calls split into per-file copies for the new layout; add a `copyDir` helper that walks `commands/brain/` and `commands/codebrain/` and emits an atomic write per file. Preserves the version marker on every file. |
...
| `.claude/prds/codebrain.prd.md` | UPDATE | v0.2 Roadmap: add M#12 row; note the M#12 → M#10 → M#11 ordering (M#12 lands first so M#10 and M#11 ship into the new layout natively) |

## Sub-milestone split (recommended)

3-way split mirrors how we handled M#3 in v0.1:

- **M#12a — Extract shared procedures into skills** (prep, no behavior change): pull v0.1.2's "How to refresh `.brain/llms.txt`" out of brain.md/codebrain.md into a new `skills/ingestion/llms-txt/SKILL.md`. Update the three callsites (M#3a Step 6, M#3b L5, lint L7) to reference the skill instead of the inline section. Tests still pass; structure unchanged.
- **M#12b — Split + mirror** (the bulk): move every verb procedure to `commands/brain/<verb>.md` and `commands/codebrain/<verb>.md`. Reduce top-level `brain.md` and `codebrain.md` to no-arg help disambiguators. Add `init.js` directory-copy support. Update tests.
...
1. Create `skills/ingestion/llms-txt/SKILL.md`:
   - Frontmatter: `name: llms-txt`, `tier: ingestion`, `pattern: Generator`, `related_skills: [behavioral/codebrain, ingestion/page-format]`
...
2. Update `commands/brain.md` and `commands/codebrain.md`:
...
3. Update `tests/e2e-test.sh` T37: replace "brain.md has refresh procedure section" with "skills/ingestion/llms-txt/SKILL.md exists with proper frontmatter and the refresh procedure"; preserve the byte-identity assertion across brain.md/codebrain.md (now trivial since the section was removed).
...
6. Mirror to `commands/codebrain/` — byte-identical body, frontmatter description swaps `/brain:` for `/codebrain:`.
...
8. Reduce `commands/codebrain.md` symmetrically.
...
10. Update `scripts/init.js`:
    ...
    - Replace the two `copyTemplate('commands/brain.md', ...)` and `copyTemplate('commands/codebrain.md', ...)` calls with: copy the two top-level files PLUS `copyDir('commands/brain', ...)` PLUS `copyDir('commands/codebrain', ...)`.
...
11. Update `tests/e2e-test.sh`:
    ...
    - T12 (alias parity for init): assert `commands/brain/init.md` and `commands/codebrain/init.md` procedure bodies are byte-identical.
    ...
    - **New T40**: structural validation of the namespacing — for each verb in `{init, ingest, query, lint, learn, status}`, both `commands/brain/<verb>.md` and `commands/codebrain/<verb>.md` exist + have frontmatter + version marker + identical procedure body.
...
14. Update `.claude/prds/codebrain.prd.md`: append M#12 row to v0.2 Roadmap; note the ordering (M#12 before M#10 before M#11).
...
for v in init ingest query lint learn status; do
  ...
  test -f "commands/codebrain/$v.md"
  head -1 "commands/brain/$v.md" | grep -qF "codebrain v"
done

# Byte-identical procedure bodies (alias parity)
for v in init ingest query lint learn status; do
  body_brain=$(awk '/^# \//{flag=1; next} flag' "commands/brain/$v.md")
  body_cb=$(awk '/^# \//{flag=1; next} flag' "commands/codebrain/$v.md")
  [ "$body_brain" = "$body_cb" ]
done
...
npm pack --dry-run | grep -q 'commands/codebrain/ingest.md'
```
...
| Claude Code's namespace separator changes upstream (e.g., from `:` to `/`) | Low | Codebrain doesn't control the separator. If Anthropic changes it, the SKILL.md docs update; the file structure on disk is unchanged. |
...
| Operators write custom slash commands under `.claude/commands/brain/<custom>.md` that conflict with future codebrain verbs | Low | Document the `commands/brain/` namespace as codebrain-owned. Future M#9-style runtime probe could detect conflicts; out of scope for M#12. |
