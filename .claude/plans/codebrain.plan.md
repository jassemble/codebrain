# Plan: codebrain — Milestone #1 (npm package skeleton + init)
...
Bootstrap codebrain as a publishable npm package. After `npx codebrain init`, the user has `.brain/` scaffolded in their cwd, `/brain *` slash commands available in Claude Code (project-local under `<cwd>/.claude/commands/` by default; `~/.claude/commands/` with `--global`), and codebrain hooks merged into `.claude/settings.local.json`. Every `/brain` verb resolves but prints "not yet implemented — Milestone N" stubs in this milestone; real behavior fills in via Milestones #2–#7. The init flow is idempotent (re-run safe), creates a `.bak` before any destructive edit, and refuses to run in a non-project directory unless `--global` is passed.

## Patterns to Mirror

| Category | Source | Pattern |
|---|---|---|
| npm package shape | `/Users/dev/Desktop/Project/OSS/idea/graphbrain/package.json` | `name`, `version`, `bin: { codebrain: "./bin/codebrain.js" }`, `license: "MIT"`, `engines: { node: ">=18" }`, `files: [...]` whitelist (don't ship `.claude/`, `reference/` to npm registry — only what `init` needs to copy) |
...
| CLI entry point | graphbrain `bin/graphbrain` (bash dispatcher; codebrain uses Node not bash) | `#!/usr/bin/env node` shebang; minimal verb dispatch — `init`, `version`, `help`, fallback shows help |
...
| File naming | `/Users/dev/Desktop/Project/OSS/idea/ECC/.claude/rules/node.md` | lowercase-with-hyphens (`brain.md`, `codebrain.js`, not `Brain.md` or `brain_init.md`) |

**Patterns we are NOT mirroring (intentional):**
- ECC's `.claude-plugin/plugin.json` + `marketplace.json` + `PLUGIN_SCHEMA_NOTES.md` — codebrain is **not** a Claude Code plugin (Design Decision #28).
- ECC's `install.sh` shell-wrapper + `install-apply.js` Node runner — codebrain's `init` is a single Node script invoked via `npx codebrain init`. No shell wrapper.
- graphbrain's `postinstall.js` (npm postinstall hook that auto-runs init) — codebrain requires an *explicit* `codebrain init` step. Auto-running on npm install pollutes user state without consent.
...
| `bin/codebrain.js` | CREATE | CLI entry: shebang `#!/usr/bin/env node`; verb dispatch (`init` | `version` | `help`); ≤80 lines |
| `scripts/init.js` | CREATE | Core init logic: target detection (project-local vs `--global`); project-dir guard; copy `commands/*.md` → `.claude/commands/`; JSON-merge codebrain hooks into `.claude/settings.local.json`; scaffold `.brain/`; append `## codebrain` block to `CLAUDE.md` inside a managed region; idempotent; `.bak` before destructive edits |
...
| `commands/codebrain.md` | CREATE | Alias template (identical body) |
| `skills/behavioral/codebrain/SKILL.md` | CREATE | Meta skill describing codebrain — bundled in the npm package; init copies into target's skill location |
...
| `LICENSE` | CREATE | MIT |
| `README.md` | CREATE | OSS-facing: install (`npx codebrain init`), quickstart, link to PRD, roadmap; ~150 lines |
...
| `tests/e2e-test.sh` | CREATE | Run `npx codebrain init` in a tmpdir (`mktemp -d`); assert `.brain/`, `.claude/commands/brain.md`, `.claude/settings.local.json` hook entries; runtime <5s |
| `.claude/prds/codebrain.prd.md` | UPDATE (done) | Milestone #1 row flipped to `in-progress` with plan link |

**Files explicitly NOT created in Milestone #1:**
- `hooks/` directory in codebrain itself — the *content* of the hooks block lives inline in `scripts/init.js` (the merge template) until Milestone #4 ships real hooks.
...
- **Action**: Create `package.json` with:
  - `name: "codebrain"` (verify availability on npm before publish)
  - `version: "0.1.0"`
  - `description`: one-line pitch from PRD hypothesis
  - `bin: { "codebrain": "./bin/codebrain.js" }`
  - `license: "MIT"`
...
### Task 2: bin/codebrain.js (CLI entry)

- **Action**: Create `bin/codebrain.js` with:
  ...
  - Verb dispatch from `process.argv[2]`:
    ...
    - `update` → **stub for v0.2**: print `codebrain update is not yet implemented in v0.1. For now, re-run \`codebrain init --force\` to refresh templates after upgrading codebrain via npm.`; exit 0
    - `uninstall` → **stub for v0.2**: print `codebrain uninstall is not yet implemented in v0.1. Manual removal: (1) delete \`.brain/\` from your repo, (2) remove entries with \`id\` starting \`codebrain:\` from \`.claude/settings.local.json\`, (3) delete \`.claude/commands/{brain,codebrain}.md\`.`; exit 0
...
- **Mirror**: graphbrain's `bin/graphbrain` (the dispatch shape; codebrain's is Node not bash)
- **Validate**: `node bin/codebrain.js` prints help; `node bin/codebrain.js version` prints `0.1.0`; `node bin/codebrain.js update` and `node bin/codebrain.js uninstall` print their deferred messages; `node bin/codebrain.js init --help` prints init-specific help (delegated to scripts/init.js)
...
- **Action**: Create `scripts/init.js` as a CommonJS module exporting `function init(argv)`:
  ...
  **Plan + atomic-write protocol**:
  - **Templates copied** to `<target>/commands/`: `brain.md`, `codebrain.md`. Each copy prepends a version-marker comment line `<!-- codebrain v0.1.0 -->` so the installed version is identifiable (per Design Decision #33's spirit, applied to templates).
  ...
  - **Scaffold `<cwd>/.brain/`** (always cwd, even with `--global` — the vault is per-repo):
    ...
    - `.brain/.codebrain-version` — one line: `0.1.0` (per Design Decision #33). This is the upgrade-detection anchor.
  - **CLAUDE.md managed-region** — append to `<cwd>/CLAUDE.md` (create if missing):
    ...
    <!-- codebrain:end -->
    ```
    Skip if `<!-- codebrain:begin -->` already present (idempotency); refresh content between markers if `--force`.
  ...
  **Hooks merge contract** (per Design Decision #32):
  ...
  - For each phase array (`PreToolUse`, `PostToolUse`, `SessionStart`, `SessionEnd`, `Stop`, `PreCompact`, `UserPromptSubmit`):
    1. Partition the existing array into `[codebrain-owned]` (entries whose `id` field starts with `codebrain:`) and `[other]` (everything else — user's or other tools' hooks)
    2. Discard the `[codebrain-owned]` partition
    3. Append codebrain's current hooks (for M#1: empty — M#4 fills these in) **after** `[other]`, preserving non-codebrain order
  - Result: codebrain hooks always reflect the installed codebrain version; non-codebrain hooks are untouched.
  ...
  **Idempotency**: re-running with the same codebrain version produces no diff; emits SKIP for already-present, version-current files. `--force` ignores SKIPs.
...
- **Validate**: covered by Task 15 (E2E test) — `npx codebrain init` in a tmpdir produces the expected files; re-running shows SKIPs; `--dry-run` writes nothing; `--global` writes to a stubbed HOME; pre-existing non-codebrain hooks in settings.local.json are preserved; re-init doesn't duplicate codebrain hooks; missing `~/.claude/` produces a WARN but proceeds
...
- **Action**: Create `commands/brain.md` (this file is the **template** the init script copies into the user's `.claude/commands/brain.md`):
  - **First line**: `<!-- codebrain v0.1.0 -->` — version marker so installed templates are identifiable for debugging "which version configured this" (paired with Design Decision #33's `.brain/.codebrain-version`)
  - YAML frontmatter: `description: codebrain — agent-maintained codebase wiki (init, ingest, query, lint, learn, status)`
...
- **Validate**: file starts with `<!-- codebrain v0.1.0 -->`; YAML frontmatter on lines 2–4 parses; smoke test post-init — `/brain` shows help, `/brain init` shows "Milestone #2" stub

### Task 5: commands/codebrain.md (alias template)

- **Action**: Create `commands/codebrain.md` with body identical to `commands/brain.md` (the dispatcher logic is the same; only the slash-command name differs because the file name differs). One-line header note: "alias for `/brain`".
- **Mirror**: Same as Task 4
- **Validate**: `/codebrain ingest` produces the same output as `/brain ingest`; byte-identical body content

### Task 6: skills/behavioral/codebrain/SKILL.md (meta skill)

- **Action**: Create `skills/behavioral/codebrain/SKILL.md` with:
...
- **Validate**: `head -12 skills/behavioral/codebrain/SKILL.md` shows valid frontmatter with all 6 fields
...
- **Action**: Create `README.md` (~150 lines):
  ...
  - **Install**:
    ```
    npx codebrain init               # project-local (recommended; default)
    npx codebrain init --global      # writes to ~/.claude/ instead
    ```
...
- **Action**: Create `CLAUDE.md` for sessions working **on codebrain itself**:
...
- **Action**: Create `.gitignore`:
  ...
  - Coverage: `coverage/`
  - Explicit comment: `# DO NOT gitignore .brain/ — codebrain commits the brain by default (Design Decision #3)`
...
- **Action**: Create `tests/e2e-test.sh`:
  - Creates tmpdir via `mktemp -d`, copies codebrain source in
  - Runs `node bin/codebrain.js init` from a *different* tmpdir simulating the user's repo (with a fake `.git/` so the project-dir guard passes)
  - **Core scaffold assertions**:
    ...
    - `<user-tmpdir>/.brain/.codebrain-version` exists and contains `0.1.0` (per Design Decision #33)
    - `<user-tmpdir>/.claude/commands/brain.md` exists, starts with `<!-- codebrain v0.1.0 -->`, and body matches the source template
    - `<user-tmpdir>/.claude/commands/codebrain.md` exists with the same version marker
    - `<user-tmpdir>/.claude/settings.local.json` parses as valid JSON
    - `<user-tmpdir>/CLAUDE.md` contains `<!-- codebrain:begin -->` and `<!-- codebrain:end -->`
  - **Hooks-ownership assertions** (per Design Decision #32):
    - Pre-seed `<user-tmpdir>/.claude/settings.local.json` with a fake non-codebrain hook: `{ "hooks": { "PreToolUse": [{ "matcher": "Bash", "hooks": [...], "id": "user:pre:my-hook" }] } }`
    - Run `node bin/codebrain.js init`
    - Assert the `user:pre:my-hook` entry is **still present** post-init (non-codebrain hooks preserved)
  ...
  - **Re-init non-duplication** (per Design Decision #32):
    ...
    - Count entries in `settings.local.json` hooks arrays whose `id` starts with `codebrain:` — must match between the two runs (no duplication)
    - All output lines from the second run start with `SKIP` (idempotency)
  - **Dry-run safety**:
    - Run `node bin/codebrain.js init --dry-run` in a fresh tmpdir
    - Assert NO filesystem changes (no `.brain/`, no `.claude/commands/brain.md` written)
  - **Project-dir guard**:
    - Run `node bin/codebrain.js init` in a tmpdir with NO `.git/`, NO `package.json`, etc., without `--global`
  ...
  - **CLI verbs**:
    - `node bin/codebrain.js update` prints the deferred-to-v0.2 message and exits 0
    - `node bin/codebrain.js uninstall` prints its deferred message and exits 0
...
- **Validate**:
  - `bash tests/e2e-test.sh` exits 0 on a clean codebrain checkout
...
npm pack --dry-run | tee /tmp/codebrain-pack.txt
...
! grep -E "\.claude/|tests/|CLAUDE\.md" /tmp/codebrain-pack.txt
# Expect: exit 0 (no matches — these stay out of the published tarball)

# 2. CLI entry runs
node bin/codebrain.js version           # prints 0.1.0
node bin/codebrain.js                   # prints help
node bin/codebrain.js help              # prints help
node bin/codebrain.js bogus-verb        # prints help + non-zero exit
...
for f in \
  ...
  skills/behavioral/codebrain/SKILL.md \
  ...
  .npmignore \
  tests/e2e-test.sh \
  .claude/prds/codebrain.prd.md \
  .claude/plans/codebrain.plan.md
do
...
for f in commands/brain.md commands/codebrain.md skills/behavioral/codebrain/SKILL.md; do
...
| Operator runs `codebrain init` outside a project dir → pollutes `~` or `cwd` | Med | Project-dir guard refuses if no `.git/`, `package.json`, `pyproject.toml`, `go.mod`, `Cargo.toml`; explicit error; `--global` is the only path that writes to `~/.claude/` |
...
| `npx codebrain init` runs against a Claude Code config layout that's changed | Low | `reference/claude-code-conventions.md` is the canonical contract — if Claude Code changes, update that file then update `init.js` to match; a post-MVP CI check could fetch the current convention docs and diff |
...
| Alias file drift (`brain.md` and `codebrain.md` diverge over time) | Low | Future milestones edit `brain.md` and re-copy to `codebrain.md`; consider CI assertion of body equality post-MVP |
| Operator runs `init --global` then later wants project-local — needs to clean up | Low | Document in README; defer `codebrain uninstall` to post-MVP |
| Idempotency bug: re-running init duplicates the CLAUDE.md schema block | Med | Init checks for `<!-- codebrain:begin -->` marker before append; E2E test (Task 15) re-runs init and asserts no diff |
| `npm publish` fails due to a name conflict (someone else owns `codebrain` on npm) | Med | **Action item before publish, not for M#1**: `npm view codebrain` to check availability; fallback names: `@<scope>/codebrain`, `codebrain-cli`, `the-codebrain` |
