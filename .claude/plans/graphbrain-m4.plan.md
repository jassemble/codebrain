...
|---|---|---|
| Hook entry shape in settings.local.json | `reference/claude-code-conventions.md` (M#1 vendored canonical contract) | `{ matcher, hooks: [{ type: command, command: "...", timeout }], id: "graphbrain:...", description }` — graphbrain owns only entries with `id` starting `graphbrain:` (PRD #32) |
| Init merges hooks via id-prefix partition | `scripts/init.js:mergeHooks` (M#1) | Read existing settings.local.json; partition each phase array into graphbrain-owned + other; replace owned; preserve others — already implemented, just needs non-empty `codebrainOwnedHooks` |
| CLI verb dispatch | `bin/graphbrain.js:main` (M#1) | switch on `process.argv[2]`; M#4 adds case `hook` that requires the named subcommand script from `scripts/hooks/` |
...
| Exit-0-on-error for hooks (never block tool execution) | ECC convention + PRD implied (hooks must not surprise the operator) | Hook script: try/catch around everything; log errors to stderr with `[graphbrain]` prefix; always `exit 0` from PostToolUse; PreToolUse `verified-guard` is the ONLY graphbrain hook that can exit non-zero (to block) |
| Tests | `tests/e2e-test.sh` T1–T19 | T20: hook scripts exist + shebang + `bin/graphbrain.js` has `hook` verb. T21: init.js writes the hook entries with `graphbrain:` ids. T22: hook scripts behave correctly when fed mock tool inputs. |

## Sweep Findings (folded in)

Seven findings from the in-flight sweep of this first-draft plan:

- **D1 — Hook invocation path: `npx graphbrain hook <subcommand>`** — solves Q1 (template/script discovery from npm-installed location, raised in M#3a/M#3c sweeps). `npx` is reliable because it resolves whichever graphbrain is installed (global or project-local) and uses the package's `bin` entry. Settings.local.json entries are short, version-stable, and don't need to know absolute install paths.
...
- **D7 — Hook script must handle the "no .brain/ in this cwd" case gracefully**: many Claude Code sessions run in repos that don't have graphbrain installed. Hook fires, sees no `.brain/`, exits 0 silently. No noise, no errors.
...
| `bin/graphbrain.js` | UPDATE | Extend verb dispatch: add `hook <subcommand> [...]`. Looks up `scripts/hooks/<subcommand>.js` and requires it with `process.argv.slice(3)`. Used by settings.local.json entries: `npx graphbrain hook stale-detect`. Errors with help if subcommand unknown. |
...
| `tests/e2e-test.sh` | UPDATE | T20: hook scripts exist + shebang + `bin/graphbrain.js hook` verb dispatches. T21: a fresh `init` run now writes the two `graphbrain:*` entries into `settings.local.json` (and a re-init still produces SKIPs or only a no-op merge per #32). T22: feed mock tool-input JSON to each hook script and assert correct behavior on the fixture `.brain/`. |
| `.claude/prds/graphbrain.prd.md` | UPDATE | M#4 row → `in-progress` with plan link |
...
- **Action**: Node CommonJS script with shebang `#!/usr/bin/env node`. Reads tool input from stdin (JSON; Claude Code hook protocol). Extracts the edited file path from tool input's `file_path`, `path`, or `target` field (handles Edit, Write, MultiEdit shapes — be defensive about field names).
  ...
  - Checks for `cwd/.brain/.graphbrain-version`. If absent → no graphbrain here → exit 0 silently.
  ...
  - Logs to stderr with `[graphbrain]` prefix on errors (never stdout — that's hook channel).
...
- **Action**: Node CommonJS script with shebang. Reads tool input from stdin. Extracts the target file path.
  ...
  - Otherwise: print to stderr:
    ```
    [graphbrain] BLOCKED: refusing to overwrite VERIFIED page <path>.
...
### Task 4: Extend bin/graphbrain.js with `hook` verb

- **Action**: Add `case 'hook'` to the main switch. Subcommand dispatch:
  ```
  graphbrain hook <subcommand> [...]   → require('../scripts/hooks/<subcommand>.js') with process.argv.slice(3)
  ```
  ...
  ```
- **Mirror**: `bin/graphbrain.js:main` (existing verb dispatch shape)
- **Validate**: T20 — `node bin/graphbrain.js hook` prints subcommand list. `node bin/graphbrain.js hook bogus` exits 1. `echo '{}' | node bin/graphbrain.js hook stale-detect` exits 0 in a directory without `.brain/`.
...
- **Validate**: T21 — run `npx graphbrain init` in a tmpdir; assert `settings.local.json` has both `graphbrain:` entries with correct `matcher`/`command`/`id`/`description`. Re-run: same content (no duplication).
...
- **Action**: Three new test sections.
  ...
  **T20 — Hook scripts exist + CLI verb dispatches:**
  ...
  - `node bin/graphbrain.js hook` prints both subcommands in help
  - `node bin/graphbrain.js hook bogus-name` exits 1
  - npm pack includes all three new hook files

  **T21 — init.js writes the graphbrain hook entries:**
  - In an existing test fixture dir, run `init`; assert `settings.local.json` has `hooks.PreToolUse` and `hooks.PostToolUse` arrays each containing an entry with `id` starting `graphbrain:`
  - Assert the `command` field references `npx graphbrain hook <subcommand>`
  - Re-run init; count of `graphbrain:*` entries unchanged (idempotent)
  - Seed settings.local.json with a user hook (id `user:foo`) + a stale graphbrain entry (id `graphbrain:old`); run init; assert `user:foo` preserved + `graphbrain:old` removed + new entries added (extends T2/T3 from M#1)

  **T22 — Hook script behavior on fixture .brain/:**
  - Set up tmpdir with `.brain/{code,concepts}/`, `.brain/.graphbrain-version`, and one fake `.brain/code/src/auth.ts.md` with `status: FRESH` + matching `source: src/auth.ts`
...
- **Action**: Edit `.claude/prds/graphbrain.prd.md`: M#4 row `pending` → `in-progress`; Plan → link to this file.
- **Validate**: `grep "4-tier staleness" .claude/prds/graphbrain.prd.md` shows updated row.
...
# 3. CLI verb wired
node bin/graphbrain.js hook 2>&1 | grep -q 'stale-detect'
node bin/graphbrain.js hook 2>&1 | grep -q 'verified-guard'

# 4. init.js merges the graphbrain entries
node -e "
...
const pre = (s.hooks.PreToolUse || []).find(e => e.id === 'graphbrain:pre:verified-guard');
const post = (s.hooks.PostToolUse || []).find(e => e.id === 'graphbrain:post:stale-detect');
...
# 6. Manual smoke (post-commit; in a real repo with graphbrain init done):
...
| Hook fires in a non-graphbrain repo and emits errors | High if unguarded | D7 — first thing the hook does is check for `.brain/.graphbrain-version`; if absent, exit 0 silently |
| `npx graphbrain hook ...` is slow on first invocation (npx cold-start) | Med | npx caches the package; subsequent invocations are fast. Document in CLAUDE.md that the first edit after `npx graphbrain init` may have a brief pause. M#4+ can investigate using `node_modules/.bin/graphbrain hook ...` if cold-start is annoying. |
...
| `lib/page-io` becomes a fragile YAML parser | Low | Only 4 fields needed (`kind`, `status`, `source`, `sources:`); custom parser handles them; if graphbrain ever needs full YAML, vendor a tiny one — but not today |
| User runs `npm pack` then `npm install` of the tarball in CI; `scripts/hooks/` doesn't get the executable bit | Med | The `files:` whitelist in package.json includes `scripts/`; the shebang is enough — npm install preserves shebangs, and `npx graphbrain hook ...` invokes via node anyway, not via the shebang. So executable bit doesn't matter. |
| Alias parity between brain.md and graphbrain.md not relevant for M#4 (no slash-command body changes) | N/A | M#4 doesn't touch commands/*.md |
...
- [ ] (Optional) Manual smoke test: edit a tracked source file in a graphbrain-initialized repo; verify the corresponding `.brain/code/` page status flips to STALE
