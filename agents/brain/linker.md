...
- **ALWAYS walk one-level transitive imports for every primary source** when building a concept's `sources:` array (v1.0.14). Use the `scripts/lib/walk-imports.js` helper to enumerate in-vault imports of each primary; add them as `sources:` entries with `transitive: true`. **Why this is load-bearing**: the M#4 staleness hook propagates only along recorded `sources:` and wikilinks. If a primary source imports from a file the concept doesn't cite in its body (e.g., `workers.service.ts` imports `streak-bonus.ts`), an edit to that transitive file won't flip the concept STALE — silent drift. Transitive sources close the gap.
